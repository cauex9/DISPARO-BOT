...
def get_configuration() -> tuple[str, str]:
    ...
    missing = [name for name, value in (("TELEGRAM_BOT_TOKEN", token), ("PAINEL_URL", panel_url)) if not value]
    if missing:
...
def build_application(token: str | None = None) -> Application:
    ...
    application = Application.builder().token(token or configured_token).build()
...
def main() -> None:
    ...
    logger.info("Bot Telegram iniciado em polling.")
