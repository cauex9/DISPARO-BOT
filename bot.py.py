...
if __name__ == "__main__":
    ...
    # No Render, inicia o bot do Telegram como processo separado
    # e mantém o Flask como processo principal do Web Service.
    telegram_process = None
    if os.getenv("TELEGRAM_BOT_TOKEN", "").strip():
        import subprocess
        try:
            telegram_process = subprocess.Popen(
                [sys.executable, str(BASE_DIR / "telegram_bot.py")],
                cwd=str(BASE_DIR),
            )
            print(f"Telegram bot iniciado (PID {telegram_process.pid}).", flush=True)
        except Exception as exc:
            print(f"ERRO ao iniciar Telegram bot: {exc}", flush=True)
    else:
        print("TELEGRAM_BOT_TOKEN ausente; Telegram não iniciado.", flush=True)
